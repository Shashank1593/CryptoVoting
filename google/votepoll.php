...
// fetching answer andoptions from poll by using id
 ...
 $res=mysql_query($qry);
 $ro1w=mysql_num_rows($res);
if($ro1w<=0)
{
            $msg3="NO POLL TO VOTE,TRY AFTER SOME TIME";
            ?>
            <script type='text/javascript'>alert('<?php echo $msg3 ?>');
...
if(isset($_POST['submit']))
{
    ...
    $pollqry="SELECT poll1 FROM google WHERE  email='$em'";
   ...
   if($pollchk=='')
         {
            ...
            $qrypoll="UPDATE google SET poll1='$pollto' WHERE email='$em'";
    ...
    else
        {
            ...
            ?>
            <script type='text/javascript'>alert('<?php echo $msg2 ?>');
            window.location="voter_dashboard.php";
            </script>
         <?php 
     }
